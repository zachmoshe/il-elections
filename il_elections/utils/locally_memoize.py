...
def _locally_memoize(func,
    ...
    logger.info(f'Locally memoizing output of `{func.__name__}` into \'{cache_location_path}\'.')
    
    stored_code_hash = open(code_hash_path).read() if code_hash_path.exists() else None
    ...
    if stored_code_hash != code_hash:
        ...
        #         p.unlink()
        with open(code_hash_path, 'wt') as out_file:
